...
export function EstadoEspacioBadge({ estado }: { estado: string }) {
...
export function EstadoTicketBadge({ estado }: { estado: string }) {
  const map: Record<string, BadgeTone> = {
    ACTIVO: 'info',
    PAGADO: 'success',
    ANULADO: 'danger',
...
export function EstadoAsignacionBadge({ estado }: { estado: string }) {
  const map: Record<string, BadgeTone> = {
    ACTIVA: 'success',
    SUSPENDIDA: 'warning',
    FINALIZADA: 'neutral',
